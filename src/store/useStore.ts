...
import type { Session, Teammate, TeamTask, TeammateSpec, ServerMessage } from '../../shared/types';
...
export const useStore = create<AppState>((set, get) => ({
  ...
  respondToPermission: (requestId, behavior, message) => {
    const { wsSend } = get();
    if (wsSend) {
      wsSend({ type: 'permission_response', requestId, behavior, ...(message ? { message } : {}) });
    }
    set(state => {
      const pendingPermissions = new Map(state.pendingPermissions);
