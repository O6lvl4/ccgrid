...
export function ExternalSkillSection({ onInstalled }: { onInstalled: () => void }) {
  ...
  const handleInstall = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await api.installPlugin({ source: source.trim(), alias: alias.trim() || undefined });
  ...
  }, [source, alias, api, plugins, skillSpecs, setPlugins, setSkillSpecs, addToast, onInstalled]);

  const canSubmit = source.trim().includes('/') && !submitting;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ fontSize: 11, fontWeight: 800, color: '#8b95a3', textTransform: 'uppercase', letterSpacing: 0.8 }}>Install from GitHub</div>
      <div>
        <div style={{ fontSize: 11, fontWeight: 600, color: '#8b95a3', marginBottom: 6 }}>Repository</div>
        <input
      ...
      <div>
        <div style={{ fontSize: 11, fontWeight: 600, color: '#8b95a3', marginBottom: 6 }}>Alias <span style={{ fontWeight: 400 }}>(optional)</span></div>
        <input
      ...
      {error && (
        <div style={{ padding: '10px 14px', borderRadius: 10, background: 'rgba(239, 68, 68, 0.08)', border: '1px solid rgba(239, 68, 68, 0.2)', color: '#dc2626', fontSize: 12, lineHeight: 1.5 }}>
          {error}
        </div>
      )}
      <button
